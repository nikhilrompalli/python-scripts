...
#VARIABLES
SONAR_HOST = '172.26.1.65'
SONAR_PORT = '9000'
JENKINS_HOST = '172.26.1.60'
JENKINS_PORT = '8080'
SONAR_API_TOKEN = '='
...
TASK_RESULT_PATH_OLD = "/root/email-report/scripts/tasks-old/"
...
BRANCH_NAME = ADMIN_EMAILS = JENKIN_JOB_NAME = GIT_URL = BUILD_NUMBER = JENKIN_BUILD_DURATION = JENKIN_BUILD_URL = MODULE_NAME = UPSTREAM_JOBS = ARTIFACTORY_URL = DEPENDENCY_HTML_FILE = ''
HTML_HASH = {"admin":''}
    ...
    try:
        if len(allViolations)>0:        
            for vio in allViolations:
                ...
                if "@" not in author:
                    author = author.strip() + "@org.com"
                elif "@" in author and (author.split("@")[1] != "org.com" and author.split("@")[1] != "gmail.com"):
                    author = author.split("@")[0] + "@org.com" 
                else:
                    author = author.strip()
...
def generate_report_mail(data):
    try:
        global GIT_URL,BRANCH_NAME,ADMIN_EMAILS,JENKIN_JOB_NAME,BUILD_NUMBER,MODULE_NAME,UPSTREAM_JOBS,ARTIFACTORY_URL,DEPENDENCY_HTML_FILE
        analysis_date = ''
        taskId = data["taskId"]
        ...
        if len(current_issues["issues"]) > 0:
            ...
            if generate_html(violationContent):
